...
from dataset import RadioMambaNetDataset, LocationDataset
...
class LightningRadioModel(pl.LightningModule):
    def __init__(self, model_params, training_config):
        ...
        self.model = RadioModel(
            ...
            efficient_scan_stage0=self.model_params.get('efficient_scan_stage0', True),
            step_size_stage0=self.model_params.get('step_size_stage0', 2),
            efficient_scan_stage1=self.model_params.get('efficient_scan_stage1', True),
            step_size_stage1=self.model_params.get('step_size_stage1', 2),
            efficient_scan_decoder_last=self.model_params.get('efficient_scan_decoder_last', True),
            step_size_decoder_last=self.model_params.get('step_size_decoder_last', 2)
        )
    ...
    def configure_optimizers(self):
        ...
        scheduler = optim.lr_scheduler.ReduceLROnPlateau(
            ...
            optimizer, mode='min', factor=0.1, patience=self.hparams.training_config['lr_scheduler_patience'])
...
class RadioMapSeerDataModule(pl.LightningDataModule):
    def __init__(self, data_config: dict, seed: int = 42, dataset_type: str = 'radiomamba'):
        """
        Args:
            data_config: 数据配置字典
            seed: 随机种子
            dataset_type: 数据集类型，'radiomamba' 或 'location'
        """
        super().__init__()
        ...
        if dataset_type == 'radiomamba':
            # RadioMambaNetDataset 配置
            ...
            # LocationDataset 配置
            self.dataset_params = {
                "resolution": data_config.get('resolution', 512),  # 512 or 1024
                "dir_pathloss": data_config.get('dir_pathloss'),  # If None, will auto-select based on resolution
                "dir_buildings": data_config.get('dir_buildings', '/home/zqm1/dataset/Location/cond/buildings_complete'),
                "dir_tx": data_config.get('dir_tx', '/home/zqm1/dataset/Location/cond/xy_pngs'),
                "thresh": data_config.get('thresh', 0.0),  # 确保传递 thresh 参数
                "transform": transforms.ToTensor()
            }
            # LocationDataset 会在内部创建和打乱 maps_inds
            self.maps_inds_shuffled = None
        else:
            raise ValueError(f"Unknown dataset_type: {dataset_type}. Must be 'radiomamba' or 'location'")

    def prepare_data(self):
        if self.dataset_type == 'radiomamba':
            print(f"[prepare_data] Checking dataset path: {self.dir_dataset}")
            if not os.path.exists(self.dir_dataset):
                raise FileNotFoundError(
                    f"Dataset path {self.dir_dataset} not found.")
            print(f"[prepare_data] Dataset path exists.")
        elif self.dataset_type == 'location':
            # 检查 LocationDataset 的路径
            for key in ['dir_pathloss', 'dir_buildings', 'dir_tx']:
                path = self.dataset_params.get(key)
                if path and not os.path.exists(path):
                    raise FileNotFoundError(f"LocationDataset path {path} not found.")
            print(f"[prepare_data] LocationDataset paths verified.")

    def setup(self, stage: str = None):
        print(f"[setup] Setting up data module, stage: {stage}, dataset_type: {self.dataset_type}")
        
        if self.dataset_type == 'radiomamba':
            dataset_args = {**self.dataset_params,
                            "maps_inds": self.maps_inds_shuffled}
            DatasetClass = RadioMambaNetDataset
        else:  # location
            dataset_args = {**self.dataset_params,
                            "maps_inds": self.maps_inds_shuffled}
            DatasetClass = LocationDataset
            # 打印 resolution 参数以确认传递
            if 'resolution' in dataset_args:
                print(f"[setup] LocationDataset resolution: {dataset_args['resolution']}")
        
        if stage == 'fit' or stage is None:
            print(f"[setup] Creating train dataset...")
            self.train_dataset = DatasetClass(phase="train", **dataset_args)
            print(f"[setup] Creating val dataset...")
            self.val_dataset = DatasetClass(phase="val", **dataset_args)
            print(f"[setup] Datasets created successfully.")
            if self.dataset_type == 'location':
                print(f"[setup] Train dataset resolution: {self.train_dataset.height}x{self.train_dataset.width}")
                print(f"[setup] Val dataset resolution: {self.val_dataset.height}x{self.val_dataset.width}")
        if stage == 'test' or stage is None:
            self.test_dataset = DatasetClass(phase="test", **dataset_args)
            if self.dataset_type == 'location':
                print(f"[setup] Test dataset resolution: {self.test_dataset.height}x{self.test_dataset.width}")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset_type', type=str, default=None,
                        choices=['radiomamba', 'location'],
                        help='Dataset type: radiomamba or location (overrides config)')
    ...
    # 与 test.py 保持一致，包括 EfficientScan 相关参数
    model_params = {
        ...
        'efficient_scan_stage0': config['Model'].get('efficient_scan_stage0', True),
        'step_size_stage0': config['Model'].get('step_size_stage0', 2),
        'efficient_scan_stage1': config['Model'].get('efficient_scan_stage1', True),
        'step_size_stage1': config['Model'].get('step_size_stage1', 2),
        'efficient_scan_decoder_last': config['Model'].get('efficient_scan_decoder_last', True),
        'step_size_decoder_last': config['Model'].get('step_size_decoder_last', 2)
    }
    ...
    # 获取数据集类型（命令行参数优先，然后是配置，最后默认 'radiomamba'）
    dataset_type = args.dataset_type or data_config.get('dataset_type', 'radiomamba')
    print(f"Using dataset type: {dataset_type}")

    print("Creating data module...")
    data_module = RadioMapSeerDataModule(
        data_config=data_config, seed=config['seed'], dataset_type=dataset_type)
    ...
    print("Trainer created. Starting fit...")
